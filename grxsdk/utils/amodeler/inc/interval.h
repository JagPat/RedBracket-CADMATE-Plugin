...
public:
  ...
  {
    if (interv.lower < lower)
      lower = interv.lower;
    if (interv.upper > upper)
      upper = interv.upper;
  }
...
public:
  ...
  {
    if (interv.lower.x < lower.x)
      lower.x = interv.lower.x;
    if (interv.upper.x > upper.x)
      upper.x = interv.upper.x;
    if (interv.lower.y < lower.y)
      lower.y = interv.lower.y;
    if (interv.upper.y > upper.y)
      upper.y = interv.upper.y;
  }
  ...
  {
    return lower.x <= interv.upper.x && upper.x >= interv.lower.x && lower.y <= interv.upper.y &&
           upper.y >= interv.lower.y;
  }
  ...
  bool operator<=(const Interval2d interv) const
  {
    ...
    {
      return lower.x >= interv.lower.x && upper.x <= interv.upper.x && lower.y >= interv.lower.y &&
             upper.y <= interv.upper.y;
    }
  }
...
{
  return interv.lower.x <= p.x && p.x <= interv.upper.x && interv.lower.y <= p.y && p.y <= interv.upper.y;
}
...
public:
  Interval3d()
  {
    lower.x = lower.y = lower.z = kMaxReal;
    upper.x = upper.y = upper.z = kMinReal;
  }
  Interval3d(int)
  {
  }
  ...
  void operator+=(Interval3d interv)
  {
    ...
    if (interv.upper.y > upper.y)
      upper.y = interv.upper.y;
    if (interv.lower.z < lower.z)
      lower.z = interv.lower.z;
    if (interv.upper.z > upper.z)
      upper.z = interv.upper.z;
  }
  ...
  {
    return lower.x <= interv.upper.x && upper.x >= interv.lower.x && lower.y <= interv.upper.y &&
           upper.y >= interv.lower.y && lower.z <= interv.upper.z && upper.z >= interv.lower.z;
  }
  ...
  bool operator<=(Interval3d interv) const
  {
    ...
    {
      return lower.x >= interv.lower.x && upper.x <= interv.upper.x && lower.y >= interv.lower.y &&
             upper.y <= interv.upper.y && lower.z >= interv.lower.z && upper.z <= interv.upper.z;
    }
  }
