...
public:
  ...
  void getAllEdgesSharingSameFaces(bool connectedSequenceOnly, std::vector<Edge*>& sortedEdges);
...
inline Edge::Edge() : mpVertex(NULL), mpFace(NULL), mpNext(NULL), mpPrev(NULL), mpCurve(NULL), mColor(defaultColor())
{
  mpPartner = this;
}
