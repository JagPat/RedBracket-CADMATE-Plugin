...
public:
  ...
  double distSqrd(Point3d p) const
  {
    return sqr(x - p.x) + sqr(y - p.y) + sqr(z - p.z);
  }
  double distRect(Point3d p) const
  {
    return fabs(x - p.x) + fabs(y - p.y) + fabs(z - p.z);
  }

  bool isEqual(Point3d p, double epsSqrd = epsAbsSqrd()) const
  {
    return (distSqrd(p) <= epsSqrd);
  }
  bool isEqualRect(Point3d p, double epsRect = epsAbs()) const
  {
  ...
  Point2d toPoint2d(int i1, int i2) const
  {
  ...
  operator GcGePoint3d&()
  {
    return *((GcGePoint3d*)this);
  }
  operator const GcGePoint3d&() const
  {
    return *((const GcGePoint3d*)this);
  }

  static Point3d& cast(GcGePoint3d& p)
  {
    return *(Point3d*)&p;
  }

  Point3d(const GcGePoint3d& p)
  {
    *this = *(Point3d*)&p;
  }
