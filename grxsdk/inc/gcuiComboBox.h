...
protected:
  ...
  void DrawTextAndFocusRect(
    LPDRAWITEMSTRUCT lpDrawItemStruct, CDC* dc, CRect& rItem, int i, COLORREF& fgColor, COLORREF& bgColor);
  void CreateAndSelectPalette(LPDRAWITEMSTRUCT lpDrawItemStruct, CDC* dc);
  void SetupForImageDraw(
    LPDRAWITEMSTRUCT lpDrawItemStruct, CDC* dc, CRect& rItem, CRect& rImage, COLORREF& fgColor, COLORREF& bgColor);
...
{
  GcDbObjectId oidLT;
  CGcUiLTypeRecord* pLTRec = (CGcUiLTypeRecord*)(GetItemDataPtr(sel));
  if (NULL != pLTRec)
...
protected:
  ...
  BOOL OnSelectOther(BOOL isOther2, int curSel, int& newSel) override;
  BOOL GetOtherColor(GcCmColor& defaultColor, const GcCmColor& layerColor, BOOL enableMetaColors);
...
public:
  ...
  void SetAllowByLayerByBlock(BOOL bAllow)
  {
    m_bAllowByLayerByBlock = bAllow;
  }
