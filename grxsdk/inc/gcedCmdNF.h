...
#pragma pack(push, 8)
  ...
  GCCORE_PORT int gcedCmdS(const resbuf* rb, bool forFutureUse = false, GcApDocument* pForFutureUse = NULL);

  typedef int (*GcEdCoroutineCallback)(void* pData);

  GCCORE_PORT int gcedCommandC(GcEdCoroutineCallback pCallback, void* pReturnFcnParms, int rtype, ...);
  GCCORE_PORT int gcedCmdC(GcEdCoroutineCallback pCallback, void* pReturnFcnParms, const struct resbuf* rb);
