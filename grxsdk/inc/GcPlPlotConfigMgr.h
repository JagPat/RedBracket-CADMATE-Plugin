...
public:
  ...
  GCPL_PORT GcPlPlotConfigInfo(const GCHAR* pDevName, const GCHAR* pFullPath, DeviceType devType);
...
public:
  ...
  virtual bool getDevicesList(
    GcArray<GcPlPlotConfigInfo, GcArrayObjectCopyReallocator<GcPlPlotConfigInfo>>& deviceList);
  virtual bool getStyleList(GcArray<GCHAR*>& list, long styType = kAllTypes);
  virtual void refreshList(RefreshCode refreshCode = kAll);

  virtual Gcad::ErrorStatus getCurrentConfig(GcPlPlotConfig*& pConfig);
  virtual Gcad::ErrorStatus setCurrentConfig(GcPlPlotConfig*& pConfig, const GCHAR* pDeviceName);
