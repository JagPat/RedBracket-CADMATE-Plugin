...
public:
  ...
  GCPL_PORT virtual void getLocalMediaName(const GCHAR* pCanonicalMediaName, GCHAR*& pLocalMediaName) const;
  ...
  GCPL_PORT virtual unsigned long isCustomPossible(
    double width, double height, bool bIsDimensional, bool bIsSheet, int dpi, const GCHAR* pPC3Dir) const;
  GCPL_PORT virtual bool matchMediaSize(double page_x,
                                        double page_y,
                                        double printable_w,
                                        double printable_h,
  ...
  GCPL_PORT virtual Gcad::ErrorStatus makeCustomMediaFromSizeDesc(HT_Media_Size* pSize,
                                                                  HT_Media_Description* pDesc,
                                                                  bool bSameDriver,
                                                                  bool bSavePC3,
                                                                  const GCHAR* pSourceEntry,
                                                                  const GCHAR* pSourceName,
                                                                  GCHAR*& pCanonicalSizeName,
                                                                  GCHAR*& pLocalizedSizeName,
                                                                  GCHAR*& pPMPPath,
                                                                  GCHAR*& pPC3Path,
                                                                  int dpi,
                                                                  double& scaleFactor);
...
protected:
  ...
  GcPlPlotConfig(HT_Plot_Config* pConfig, const GCHAR* pDevName, const GCHAR* pFullPath, unsigned long nDevType);
