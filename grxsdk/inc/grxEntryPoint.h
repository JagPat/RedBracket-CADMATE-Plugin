...
extern "C"
{
  __declspec(selectany) __declspec(allocate(GRXCOMMAND_FIRST_ENTRYNAME))
    _GRXCOMMAND_ENTRY* __pGrxCmdMapEntryFirst = NULL;
...
#    define GCED_GRXCOMMAND_ENTRY_PRAGMA(group, globCmd) \
      __pragma(comment(linker, "/include:__pGrxCmdMap_" #group #globCmd));
#  else
#    define GCED_GRXCOMMAND_ENTRY_PRAGMA(group, globCmd) \
      __pragma(comment(linker, "/include:___pGrxCmdMap_" #group #globCmd));
#  endif
#else
#  define GCED_GRXCOMMAND_ENTRY_PRAGMA(group, globCmd)
#endif
...
#define GCED_GRXCOMMAND_ENTRY_AUTO(classname, group, globCmd, locCmd, cmdFlags, UIContext)              \
  __declspec(selectany) _GRXCOMMAND_ENTRY __GrxCmdMap_##group##globCmd = {                              \
    _RXST(#group), _RXST(#globCmd), _RXST(#locCmd), cmdFlags, classname::group##globCmd, UIContext, 0}; \
  extern "C" __declspec(allocate("GRXCOMMAND$__m")) __declspec(selectany)                               \
    _GRXCOMMAND_ENTRY* __pGrxCmdMap_##group##globCmd = &__GrxCmdMap_##group##globCmd;                   \
...
#define GCED_GRXCOMMAND_ENTRYBYID_AUTO(classname, group, globCmd, locCmdId, cmdFlags, UIContext)     \
  __declspec(selectany) _GRXCOMMAND_ENTRY __GrxCmdMap_##group##globCmd = {                           \
    _RXST(#group), _RXST(#globCmd), NULL, cmdFlags, classname::group##globCmd, UIContext, locCmdId}; \
  extern "C" __declspec(allocate("GRXCOMMAND$__m")) __declspec(selectany)                            \
    _GRXCOMMAND_ENTRY* __pGrxCmdMap_##group##globCmd = &__GrxCmdMap_##group##globCmd;                \
  GCED_GRXCOMMAND_ENTRY_PRAGMA(group, globCmd)
...
extern "C"
{
  __declspec(selectany) __declspec(allocate(GDSSYMBOL_FIRST_ENTRYNAME))
    _GDSSYMBOL_ENTRY* __pGdsSymbolMapEntryFirst = NULL;
  __declspec(selectany) __declspec(allocate(GDSSYMBOL_LAST_ENTRYNAME))
    _GDSSYMBOL_ENTRY* __pGdsSymbolMapEntryLast = NULL;
}
...
#define GCED_GDSSYMBOL_ENTRY_AUTO(classname, name, regFunc)                                                          \
  ...
  extern "C" __declspec(allocate("GDSSYMBOL$__m")) __declspec(selectany) _GDSSYMBOL_ENTRY* __pGdsSymbolMap_##name =  \
    &__GdsSymbolMap_##name;                                                                                          \
  GCED_GDSSYMBOL_ENTRY_PRAGMA(name)
#define GCED_GDSCOMMAND_ENTRY_AUTO(classname, name, regFunc)                                                        \
  __declspec(selectany)                                                                                             \
    _GDSSYMBOL_ENTRY __GdsSymbolMap_##name = {_RXST("c:") _RXST(#name), classname::gds_##name, regFunc, -1};        \
  extern "C" __declspec(allocate("GDSSYMBOL$__m")) __declspec(selectany) _GDSSYMBOL_ENTRY* __pGdsSymbolMap_##name = \
    &__GdsSymbolMap_##name;                                                                                         \
  GCED_GDSSYMBOL_ENTRY_PRAGMA(name)
  ...
  virtual GcRx::AppRetCode On_kInitAppMsg(void* pkt)
  {
    ...
    for (_GRXCOMMAND_ENTRY** ppEntry = ppGrxCmdMapEntryFirst; ppEntry < ppGrxCmdMapEntryLast; ppEntry++)
    {
      if (*ppEntry != NULL)
      {
        ...
        gcedRegCmds->addCommand((*ppEntry)->pszCmdGroupName,
