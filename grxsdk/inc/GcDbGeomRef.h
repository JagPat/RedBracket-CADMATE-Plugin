...
protected:
  void setEntity(const GcDbCompoundObjectId& entityId)
  {
    mEntityId = entityId;
  }
  void setSubent(const GcDbSubentId& subentId)
  {
    mSubentId = subentId;
  }
...
public:
  ...
  GcDbSubentId faceSubentId() const
  {
...
public:
  ...
  explicit GcDbVertexRef(const GcDbCompoundObjectId&,
                         const GcDbSubentId& = kNullSubentId,
                         const GcGePoint3d& = GcGePoint3d::kOrigin);
  ...
  explicit GcDbVertexRef(ImpliedType,
                         const GcDbEdgeRef& edgeRef,
  ...
  const GcDbGeomRef* referencedRef() const
  {
...
public:
  ...
  GcDbPathRef(const GcDbFullSubentPathArray& edgeSubentPathArr, const GcDbFullSubentPathArray& faceSubentPathArr);
  ~GcDbPathRef();
  ...
  const GcArray<GcDbEdgeRef>& edgeRefs() const
  {
    return mEdgeRefs;
  }
...
public:
  ...
  bool isValid() const override;
  bool isEmpty() const override;
  const GcGeVector3d vector() const
  {
    return mVector;
  }
...
public:
  ...
  bool isValid() const override;
  bool isEmpty() const override;
  const GcGeMatrix3d& coordSystem() const
  {
    return mCoordSystem;
  }
  const GcDbCompoundObjectId& entityId() const
  {
    return mEntityId;
  }
  const GcArray<GcDbSubentRef*>& subentRefs() const
  {
    return mSubentRefs;
  }
