...
public:
  ...
  GcDbArrayGripAppData(const GcGePoint3d& position = GcGePoint3d(), unsigned int modes = 0);
  ~GcDbArrayGripAppData();

  const GcGePoint3d& position() const
  {
    return mPosition;
  }
  ...
  GcDbItemLocator& locator()
  {
    return mLocator;
  }

  const GcDbDimDataPtrArray& dimData() const
  {
    return mDimData;
  }
  ...
  const GcGePoint3d* basePoint() const
  {
    return mpBasePoint;
  }
