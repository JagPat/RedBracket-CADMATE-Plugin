...
public:
  ...
  DBCONSTRAINTS_PORT explicit GcDbAssocAsmBodyActionParam(
    GcDbAssocCreateImpObject createImpObject = kGcDbAssocCreateImpObject);

  DBCONSTRAINTS_PORT virtual Gcad::ErrorStatus setBody(const GcDbEntity* pAsmEntity,
                                                       bool isReadDependency = true,
                                                       bool isWriteDependency = false,
                                                       int dependencyOrder = 0);
  DBCONSTRAINTS_PORT virtual Gcad::ErrorStatus setBody(GcDbObjectId asmEntityId,
                                                       bool isReadDependency = true,
                                                       bool isWriteDependency = false,
                                                       int dependencyOrder = 0);
