...
public:
  ...
  DBCONSTRAINTS_PORT Gcad::ErrorStatus addConstrainedGeometry(const GcDbFullSubentPath& subentPath,
  ...
  DBCONSTRAINTS_PORT Gcad::ErrorStatus addRadiusDiameterConstraint(
    GcConstrainedGeometry* pConsGeom,
  ...
  DBCONSTRAINTS_PORT Gcad::ErrorStatus getAllConnectedGeomDependencies(
    const GcDbObjectIdArray& srcGeomDependencyIds, GcDbObjectIdArray& allConnectedGeomDependencyIds) const;
...
public:
  ...
  virtual void constraintDeactivated(const GcExplicitConstraint* pConstraint, bool bDeactivated = true)
  {
    UNREFERENCED_PARAMETER(pConstraint);
...
public:
  ...
  DBCONSTRAINTS_PORT virtual void getConstraintPriorityOverride(GcGeomConstraint::GeomConstraintType type,
                                                                const GcArray<const GcConstrainedGeometry*>& geometries,
                                                                int& priority) const
  {
  ...
  DBCONSTRAINTS_PORT virtual void getAutoConstrainPriority(
    GcArray<GcGeomConstraint::GeomConstraintType>& constraintList) const
  {
    constraintList.setLogicalLength(0);
    for (int type = (int)GcGeomConstraint::kHorizontal; type < (int)GcGeomConstraint::kEqualRadius; ++type)
    {
      constraintList.append((GcGeomConstraint::GeomConstraintType)type);
    }
  }

  DBCONSTRAINTS_PORT virtual bool isEvaluationCancelled()
  {
    return false;
  }
...
private:
  ...
  bool isValidConstraintType(GcGeomConstraint::GeomConstraintType type, const GcGeomConstraint* pConstraint) const;
