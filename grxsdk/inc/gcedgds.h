...
#  if (defined(_GSOFT_CROSS_PLATFORM_) && defined(GCCORE_FABRIC) && !defined(CRX_API)) || \
    !defined(_GSOFT_CROSS_PLATFORM_)
...
GCCORE_PORT int gcedDefunEx(const GCHAR* pszGlobalName, const GCHAR* pszLocalName, int nFuncNum);

GCAD_PORT int gcedSetFunHelp(const GCHAR* pszFunctionName, const GCHAR* pszHelpfile, const GCHAR* pszTopic, int iCmd);
...
GCCORE_PORT int gcedEntSel(const GCHAR* str, gds_name entres, gds_point ptres);
#ifdef __cplusplus
class GcSelectionPreview;
GCCORE_PORT int gcedEntSel(const GCHAR* str, gds_name entres, gds_point ptres, GcSelectionPreview* pSelectionPreview);
#endif
GCCORE_PORT int gcedNEntSel(
  const GCHAR* str, gds_name entres, gds_point ptres, gds_point xformres[4], struct resbuf** refstkres);
GCCORE_PORT int gcedNEntSelP(
  const GCHAR* str, gds_name entres, gds_point ptres, int pickflag, gds_matrix xformres, struct resbuf** refstkres);
GCCORE_PORT int gcedSSGet(const GCHAR* str, const void* pt1, const void* pt2, const struct resbuf* filter, gds_name ss);
#ifdef __cplusplus
GCCORE_PORT int gcedSSGet(const GCHAR* str,
                          const void* pt1,
                          const void* pt2,
                          const struct resbuf* filter,
                          gds_name ss,
                          GcSelectionPreview* pSelectionPreview);
#endif
...
#endif
GCCORE_PORT int gcedSSAdd(const gds_name ename, const gds_name sname, gds_name result);
...
GCCORE_PORT int gcedSSNameX(struct resbuf** rbpp, const gds_name ss, int i);
GCCORE_PORT int gcedSSNameXEx(struct resbuf** rbpp, const gds_name ss, int i, unsigned int flags);
...
GCCORE_PORT int gcedTrans(
  const gds_point pt, const struct resbuf* from, const struct resbuf* to, int disp, gds_point result);
...
GCCORE_PORT int gcedInvoke(const struct resbuf* args, struct resbuf** result);
...
GCCORE_PORT int gcedGetString(int cronly, const GCHAR* prompt, GCHAR* result, size_t bufsize);
...
GCCORE_PORT int gcedGetAngle(const gds_point pt, const GCHAR* prompt, gds_real* result);
GCCORE_PORT int gcedGetCorner(const gds_point pt, const GCHAR* prompt, gds_point result);
GCCORE_PORT int gcedGetDist(const gds_point pt, const GCHAR* prompt, gds_real* result);
GCCORE_PORT int gcedGetOrient(const gds_point pt, const GCHAR* prompt, gds_real* result);
GCCORE_PORT int gcedGetPoint(const gds_point pt, const GCHAR* prompt, gds_point result);
...
GCCORE_PORT int gcedOsnap(const gds_point pt, const GCHAR* mode, gds_point result);
...
GCCORE_PORT int gcedGrDraw(const gds_point from, const gds_point to, int color, int hl);
GCCORE_PORT int gcedGrVecs(const struct resbuf* vlist, gds_matrix mat);
GCCORE_PORT int gcedXformSS(const gds_name ssname, gds_matrix genmat);
GCCORE_PORT int gcedDragGen(
  const gds_name ss, const GCHAR* pmt, int cursor, int (*scnf)(gds_point pt, gds_matrix mt), gds_point p);
GCCORE_PORT int gcedSetView(const struct resbuf* view, int vport);

GCAD_PORT int gcedGetFileD(
  const GCHAR* title, const GCHAR* defawlt, const GCHAR* ext, int flags, struct resbuf* result);
GCAD_PORT int gcedGetFileNavDialog(
  const GCHAR* title, const GCHAR* defawlt, const GCHAR* ext, const GCHAR* dlgname, int flags, struct resbuf** result);

GCCORE_PORT int gcedTextBox(const struct resbuf* args, gds_point pt1, gds_point pt2);

GCCORE_PORT int gcedTablet(const struct resbuf* args, struct resbuf** result);
...
GCCORE_PORT struct resbuf* gcdbTblSearch(const GCHAR* tblname, const GCHAR* sym, int setnext);
GCCORE_PORT int gcdbNamedObjDict(gds_name result);
GCCORE_PORT struct resbuf* gcdbDictSearch(const gds_name dict, const GCHAR* sym, int setnext);
GCCORE_PORT struct resbuf* gcdbDictNext(const gds_name dict, int rewind);
GCCORE_PORT int gcdbDictRename(const gds_name dictname, const GCHAR* oldsym, const GCHAR* newsym);
GCCORE_PORT int gcdbDictRemove(const gds_name dictname, const GCHAR* symname);
GCCORE_PORT int gcdbDictAdd(const gds_name dictname, const GCHAR* symname, const gds_name newobj);

GCCORE_PORT int gcdbTblObjName(const GCHAR* tblname, const GCHAR* sym, gds_name result);
...
GCCORE_PORT int gcdbInters(
  const gds_point from1, const gds_point to1, const gds_point from2, const gds_point to2, int teston, gds_point result);
...
inline int gcedGetEnv(const wchar_t* pszName, wchar_t (&buf)[nBufLen])
{
  return ::gcedGetEnv(pszName, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcedGetInput(wchar_t (&buf)[nBufLen])
{
  return ::gcedGetInput(buf, nBufLen);
}

template<size_t nBufLen>
inline int gcedGetString(int cronly, const wchar_t* prompt, wchar_t (&buf)[nBufLen])
{
  return ::gcedGetString(cronly, prompt, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcedFindFile(const wchar_t* pszName, wchar_t (&buf)[nBufLen])
{
  return ::gcedFindFile(pszName, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcedFindTrustedFile(const wchar_t* pszName, wchar_t (&buf)[nBufLen])
{
  return ::gcedFindTrustedFile(pszName, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcedGetKword(const wchar_t* pszPrompt, wchar_t (&buf)[nBufLen])
{
  return ::gcedGetKword(pszPrompt, buf, nBufLen);
}

template<size_t nPreBufLen, size_t nNameBufLen, size_t nExtBufLen>
inline int gcedFNSplit(const wchar_t* pathToSplit,
                       wchar_t (&prebuf)[nPreBufLen],
                       wchar_t (&namebuf)[nNameBufLen],
                       wchar_t (&extbuf)[nExtBufLen])
{
  return ::gcedFNSplit(pathToSplit, prebuf, nPreBufLen, namebuf, nNameBufLen, extbuf, nExtBufLen);
}

template<size_t nNameBufLen>
inline int gcedFNSplit(const wchar_t* pathToSplit, size_t nPre, wchar_t (&namebuf)[nNameBufLen], size_t nExt)
{
  return ::gcedFNSplit(pathToSplit, nullptr, nPre, namebuf, nNameBufLen, nullptr, nExt);
}

template<size_t nExtBufLen>
inline int gcedFNSplit(const wchar_t* pathToSplit, size_t nPre, size_t nName, wchar_t (&extbuf)[nExtBufLen])
{
...
inline int gcedFNSplit(const wchar_t* pathToSplit, wchar_t (&prebuf)[nPreBufLen], size_t nName, size_t nExt)
{
  return ::gcedFNSplit(pathToSplit, prebuf, nPreBufLen, nullptr, nName, nullptr, nExt);
}

template<size_t nBufLen>
inline int gcdbAngToS(gds_real v, int unit, int prec, wchar_t (&buf)[nBufLen])
{
  return ::gcdbAngToS(v, unit, prec, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcdbRawAngToS(gds_real v, int unit, int prec, wchar_t (&buf)[nBufLen])
{
  return ::gcdbRawAngToS(v, unit, prec, buf, nBufLen);
}

template<size_t nBufLen>
inline int gcdbRToS(gds_real v, int unit, int prec, wchar_t (&buf)[nBufLen])
{
