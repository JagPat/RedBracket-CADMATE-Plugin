                              ...
                              GCHAR*& pszFieldCode)
{
  GcString sFieldCode;
  const bool bRet =
    ::GcFdMakeFieldCode(objId, blockParamId, pszPropOrConnectionName, nOption, pszFormat, pHyperlink, sFieldCode);
                              ...
                              GCHAR*& pszFieldCode)
{
  GcString sFieldCode;
  const bool bRet =
    ::GcFdMakeFieldCode(pSmDb, pSmSheetSet, pSmComp, pszCompName, pszPropName, nOption, pszFormat, sFieldCode);
  pszFieldCode = nullptr;
  if (bRet)
    ::gcutNewString(sFieldCode.kwszPtr(), pszFieldCode);
  return bRet;
...
{
  GcString sFieldCode;
  const bool bRet = ::GcFdMakeFieldCode(tableId, targetTableId, nOption, cellRanges, pszFormat, pHyperlink, sFieldCode);
  pszFieldCode = nullptr;
  if (bRet)
    ::gcutNewString(sFieldCode.kwszPtr(), pszFieldCode);
  return bRet;
                            ...
                            GCHAR*& pszPropName)
{
  ...
  const bool bRet = ::GcFdExtractData(pField, sSheetSetFile, sSheetSetId, sCompName, sCompId, sPropName);
  if (!bRet)
...
GCAD_PORT bool GcFdExtractData(GcDbField* pField, GcDbObjectId& objId, GcString& sPropName);
inline bool GcFdExtractData(GcDbField* pField, GcDbObjectId& objId, GCHAR*& pszPropName)
{
  GcString sPropName;
