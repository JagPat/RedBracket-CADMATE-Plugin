...
#define GCRX_DEFINE_GPDESC(CLASS_NAME, AUTOINIT) GcRxClass* CLASS_NAME::gpDesc = AUTOINIT(CLASS_NAME);

#define GCRX_DECLARE_GPDESC(CLASS_NAME) static GcRxClass* gpDesc;

class GcRxObject;
template<typename T>
static GcRxObject* gcrxInstantiateClass()
{
  return new T();
}
...
#define GCRX_DECLARE_MEMBERS_EXPIMP(CLASS_NAME, EXPIMP)                                           \
  ...
  {                                                                                               \
    return ((inPtr == NULL) || !inPtr->isKindOf(CLASS_NAME::desc())) ? NULL : (CLASS_NAME*)inPtr; \
  };                                                                                              \
  static void rxInit();                                                                           \
  static void rxInit(AppNameChangeFuncPtr);                                                       \
  static const wchar_t* className()                                                               \
  {                                                                                               \
    return GCRX_T(#CLASS_NAME);                                                                   \
  }

#define GCRX_EMPTY

#define GCRX_DECLARE_MEMBERS(CLASS_NAME) GCRX_DECLARE_MEMBERS_EXPIMP(CLASS_NAME, GCRX_EMPTY)
...
#define GCRX_DEFINE_MEMBERS(CLASS_NAME) GCRX_DEFINE_MEMBERS_WORKER(CLASS_NAME, GCRX_AUTO_INIT_DEFAULT)
...
#define GCRX_NO_CONS_DEFINE_MEMBERS_WITH_PROPERTIES_WORKER(AUTOINIT, CLASS_NAME, PARENT_CLASS, MAKEPROPS) \
  ...
  {                                                                                                       \
    GCRX_STATIC_CHECK(CLASS_NAME);                                                                        \
    gpDesc = newGcRxClass(GCRX_T(#CLASS_NAME), GCRX_T(#PARENT_CLASS), 0, 0, 0, 0, 0, 0, 0, MAKEPROPS);    \
  }
...
#define GCRX_CONS_DEFINE_MEMBERS_WORKER(AUTOINIT, CLASS_NAME, PARENT_CLASS, VERNO)                               \
  ...
  {                                                                                                              \
    GCRX_STATIC_CHECK(CLASS_NAME);                                                                               \
    gpDesc = newGcRxClass(GCRX_T(#CLASS_NAME), GCRX_T(#PARENT_CLASS), VERNO, &gcrxInstantiateClass<CLASS_NAME>); \
  }
...
#define GCRX_DXF_DEFINE_MEMBERS(                                                          \
  CLASS_NAME, PARENT_CLASS, DWG_VERSION, MAINTENANCE_VERSION, PROXY_FLAGS, DXF_NAME, APP) \
  GCRX_DEFINE_MEMBERS(CLASS_NAME);                                                        \
  void CLASS_NAME::rxInit()                                                               \
  {                                                                                       \
    GCRX_STATIC_CHECK(CLASS_NAME);                                                        \
    gpDesc = newGcRxClass(GCRX_T(#CLASS_NAME),                                            \
                          GCRX_T(#PARENT_CLASS),                                          \
                          DWG_VERSION,                                                    \
                          MAINTENANCE_VERSION,                                            \
                          PROXY_FLAGS,                                                    \
                          &gcrxInstantiateClass<CLASS_NAME>,                              \
                          GCRX_T(#DXF_NAME),                                              \
                          GCRX_T(#APP));                                                  \
  }                                                                                       \
...
#define GCRX_DXF_DEFINE_MEMBERS_WITH_PROPERTIES(                                                     \
  CLASS_NAME, PARENT_CLASS, DWG_VERSION, MAINTENANCE_VERSION, PROXY_FLAGS, DXF_NAME, APP, MAKEPROPS) \
  GCRX_DEFINE_MEMBERS(CLASS_NAME);                                                                   \
  void CLASS_NAME::rxInit()                                                                          \
  {                                                                                                  \
    GCRX_STATIC_CHECK(CLASS_NAME);                                                                   \
