...
{
  friend void GetDisplayNameHelper(const CGdUiPathname* pPathName, GcString& strResult, int maxchars);
  friend CGdUiVolumeDescriptor* NewVolDescriptorHelper(const CGdUiPathname* pPathName, const GcString& vol_name);
  friend bool IsPathValid(CGdUiPathname* pPathName, const CGdUiPathname* rel_path, const GcString* work);
  friend void HandleSpecialPath(
    CGdUiPathname* pPathName, const CGdUiPathname* rel_path, GcString* work, int index, bool wildcard_ok);
  friend bool IsValidOnColon(const GcString* work);
  friend void SetupVolForRoottypePath(CGdUiPathname* pPathName, const CGdUiPathname* pRelPath, const wchar_t* pPathStr);
...
public:
  ...
  PathErr ParseRelative(const wchar_t* name, const CGdUiPathname* relpath, bool wildcard_ok = false);
  bool ParseAndOSFind(const wchar_t*);

  const CGdUiPathname& operator=(const CGdUiPathname& pathSrc);
  const CGdUiPathname& operator=(const CGdUiPathname* pathSrc);

  bool GetUniqueName(CGdUiPathname&, bool alwaysMangle = false) const;
...
private:
  ...
  void GetAttributes(const GcString* path, const GcString* relpath, path_type* this_type);
};
...
{
  return ((m_this_type == FILE_PATH) || (m_this_type == DIR_PATH) || (m_this_type == NEW_PATH) ||
          (m_this_type == WC_PATH));
}
