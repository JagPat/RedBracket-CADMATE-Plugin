...
inline GcAxObjectRef::GcAxObjectRef() : m_objId(NULL), m_pObj(NULL)
{
}

inline GcAxObjectRef::GcAxObjectRef(GcDbObjectId objId) : m_objId(NULL), m_pObj(NULL)
{
  acquire(objId);
}

inline GcAxObjectRef::GcAxObjectRef(GcDbObject*& pObj) : m_objId(NULL), m_pObj(NULL)
{
  acquire(pObj);
}
...
template<class T_OBJECT>
inline GcAxObjectRefPtr<T_OBJECT>::GcAxObjectRefPtr(GcAxObjectRef* const pRef, GcDb::OpenMode mode, bool openErased)
  : m_pRef(pRef)
{
  if (NULL == pRef)
  {
