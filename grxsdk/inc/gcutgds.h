...
GCCORE_PORT int gcutCvUnit(gds_real value, const GCHAR* oldunit, const GCHAR* newunit, gds_real* result);
...
GCCORE_PORT void gcutPolar(const gds_point pt, gds_real angle, gds_real dist, gds_point ptres);
...
inline int gcutSPrintf(wchar_t (&buf)[nBufLen], const GCHAR* format, ...)
{
  va_list va;
  va_start(va, format);
...
template<size_t nBufLen>
inline int gcutVSPrintf(wchar_t (&buf)[nBufLen], const GCHAR* format, va_list va)
{
