...
public:
  ...
  static Gcad::ErrorStatus fitPointsToImage(GcGePoint2dArray& pointArray,
...
private:
  ...
  void drawFrame(GcGiViewportDraw* mode, const GcGePoint3dArray& verts) const;
  Gcad::ErrorStatus setDcToScreen(
    const GcGePoint2d& lowerLeft, const GcGePoint2d& upperRight, int width, int height, GcGeMatrix2d& dcToScreen) const;
  void setFrameExtents(GcGiWorldDraw* mode, const GcGePoint3dArray& verts) const;
  bool drawFilledPloygon(GcGiViewportDraw* mode, const GcGePoint3dArray& verts) const;
...
private:
  ...
  void* operator new[](size_t nSize, const char* file, int line) throw()
  {
    return 0;
  }
};
