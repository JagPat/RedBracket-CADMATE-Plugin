...
public:
  ...
  Gcad::ErrorStatus addMoreObjectsToDeepCloneOverride(GcDbIdMapping&,
  ...
  static Gcad::ErrorStatus findActionsThatAffectedTopologicalSubentity(const GcDbEntity* pAsmBasedEntity,
                                                                       const GcDbSubentId&,
                                                                       GcDbObjectIdArray& actionIds);

  static Gcad::ErrorStatus getTopologicalSubentitiesForActionsOnEntity(const GcDbEntity* pAsmBasedEntity,
                                                                       bool alsoUseAdjacentTopology,
                                                                       GcDbObjectIdArray& actionIds,
                                                                       GcArray<GcArray<GcDbSubentId>>& actionSubentIds);

  static Gcad::ErrorStatus getSurfacesDirectlyDependentOnObject(const GcDbObject*,
                                                                GcDbObjectIdArray& dependentSurfaceIds);

  virtual Gcad::ErrorStatus getGripPoints(GcGePoint3dArray& gripPoints,
                                          GcDbIntArray& osnapModes,
                                          GcDbIntArray& geomIds) const;

  virtual Gcad::ErrorStatus getGripPoints(GcDbGripDataPtrArray& grips,
                                          const double curViewUnitSize,
                                          const int gripSize,
                                          const GcGeVector3d& curViewDir,
                                          const int bitflags) const;

  virtual Gcad::ErrorStatus moveGripPoints(const GcDbIntArray& indices, const GcGeVector3d& offset);

  virtual Gcad::ErrorStatus moveGripPoints(const GcDbVoidPtrArray& gripAppData,
                                           const GcGeVector3d& offset,
                                           const int bitflags);

  virtual Gcad::ErrorStatus moveStretchPoints(const GcDbIntArray& indices, const GcGeVector3d& offset);

  virtual void gripStatus(const GcDb::GripStat status);

  virtual Gcad::ErrorStatus getGripEntityUCS(const void* pGripAppData,
