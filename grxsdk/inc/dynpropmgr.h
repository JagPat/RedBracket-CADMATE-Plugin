...
#else
#  define OPM_DYNPROP_OBJECT_ENTRY_PRAGMA(classname, rxclass) \
    __pragma(comment(linker, "/include:___pOpmObjMap_" #rxclass #classname));
#endif
...
#define OPM_DYNPROP_OBJECT_ENTRY_MAKE(classname, rxclass, clsid)                                     \
  ...
  extern "C" __declspec(allocate("OPM_DYNPROP$__m")) __declspec(selectany)                           \
    _OPM_DYNPROP_ENTRY* const __pOpmObjMap_##rxclass##classname = &__OpmObjMap_##rxclass##classname; \
...
#else
#  define OPM_DYNPROP_COMMAND_ENTRY_PRAGMA(classname, cmdname) \
    __pragma(comment(linker, "/include:___pOpmObjMap_" #cmdname #classname));
#endif
...
#define OPM_DYNPROP_COMMAND_ENTRY_MAKE(classname, cmdname, clsid)                                    \
  ...
  extern "C" __declspec(allocate("OPM_DYNPROP$__m")) __declspec(selectany)                           \
    _OPM_DYNPROP_ENTRY* const __pOpmObjMap_##cmdname##classname = &__OpmObjMap_##cmdname##classname; \
    ...
    for (_OPM_DYNPROP_ENTRY** ppEntry = m_ppOpmObjMapEntryFirst; ppEntry < m_ppOpmObjMapEntryLast; ppEntry++)
    {
      if (*ppEntry != NULL)
      {
        if (*((*ppEntry)->pclsid) == __uuidof(IDynamicProperty2) || *((*ppEntry)->pclsid) == __uuidof(IDynamicProperty))
        {
          ...
          {
            if ((pPropMan =
                   GET_OPMEXTENSION_CREATE_PROTOCOL()->CreateOPMObjectProtocol(NULL, 2)->GetPropertyManager()) == NULL)
          ...
          else
          {
            ...
            OPMPropertyExtension* pOPMExt =
              GET_OPMEXTENSION_CREATE_PROTOCOL()->CreateOPMCommandProtocol((*ppEntry)->strCmd);
            pOPMExt->SetPropertyManager(pPropMan);
          }
          _com_util::CheckError(
            (*ppEntry)->pfnCreateInstance(NULL, *((*ppEntry)->pclsid), (void**)&((*ppEntry)->pProp)));
        ...
        else if (*((*ppEntry)->pclsid) == __uuidof(IPropertySource))
        {
          ...
          _com_util::CheckError(
            (*ppEntry)->pfnCreateInstance(NULL, *((*ppEntry)->pclsid), (void**)&((*ppEntry)->pPropSrc)));
          CComBSTR propName;
          ...
          if (GET_OPM_PERINSTANCE_EXTENSION_PROTOCOL((*ppEntry)->pClass)->AddObjectPropertySourceName(&propName) ==
              false)
            _com_issue_error(E_FAIL);
        }
      }
    ...
    for (_OPM_DYNPROP_ENTRY** ppEntry = m_ppOpmObjMapEntryFirst; ppEntry < m_ppOpmObjMapEntryLast; ppEntry++)
    {
      if (*ppEntry != NULL)
      {
        ...
        else if (*((*ppEntry)->pclsid) == __uuidof(IPropertySource))
        {
          ...
          CComBSTR propName;
          (*ppEntry)->pPropSrc->get_Name(&propName);
          if (GET_OPM_PERINSTANCE_EXTENSION_PROTOCOL((*ppEntry)->pClass)->RemoveObjectPropertySourceName(&propName) ==
              false)
            _com_issue_error(E_FAIL);
