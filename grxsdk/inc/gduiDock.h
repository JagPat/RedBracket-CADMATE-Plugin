...
public:
  ...
  const CSize& GetHorzDockSize()
  {
    return m_HorzDockSize;
  }
  const CSize& GetVertDockSize()
  {
    return m_VertDockSize;
  }
  void SetHorzDockSize(const CSize& size)
  {
    m_HorzDockSize = size;
  }
  void SetVertDockSize(const CSize& size)
  {
    m_VertDockSize = size;
  }
...
private:
  ...
  void SnapToRect(
    CRect& rect, WINDOWPOS* pWndpos, int nPaletteWidth, int nPaletteHeight, bool bSnapInside, bool bSnapOutside);
  void InitFonts();
  ...
  DWORD GetRolloutDelay()
  {
    return m_nRolloutDelay;
  }
...
bool GCAD_PORT GdUiRegisterDockingTool(LPCTSTR lpszToolName, LPCTSTR lpszCommand, UINT nToolbarID);
bool GCAD_PORT GdUiRegisterTool(LPCTSTR lpszToolName, LPCTSTR lpszCommand, CLSID* pID);
...
#ifndef _GSOFT_MAC_
void GCAD_PORT GdUiEnableDocking(CFrameWnd* pFrame, DWORD dwDockStyle, GDUI_LOADFN* pLoadFcn, GDUI_SAVEFN* pSaveFcn);
