...
public:
  ...
  DBCONSTRAINTS_PORT explicit GcDbAssocExtrudedSurfaceActionBody(
    GcDbAssocCreateImpObject createImpObject = kGcDbAssocCreateImpObject);

  DBCONSTRAINTS_PORT Gcad::ErrorStatus setTaperAngle(double dTaperAngle,
                                                     const GcString& expression = GcString(),
                                                     const GcString& evaluatorId = GcString());
  DBCONSTRAINTS_PORT double taperAngle(GcString& expression = dummyString(),
                                       GcString& evaluatorId = dummyString()) const;

  DBCONSTRAINTS_PORT Gcad::ErrorStatus setHeight(double dHeight,
                                                 const GcString& expression = GcString(),
                                                 const GcString& evaluatorId = GcString());
