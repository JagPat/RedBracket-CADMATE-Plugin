...
public:
  ...
  Gcad::ErrorStatus evaluate(int nContext, GcDbDatabase* pDb, int* pNumFound = NULL, int* pNumEvaluated = NULL);
...
#error  getHyperlink(GCHAR**,GCHAR**,GCHAR**,GCHAR**,int) should not be ported anymore. Please remove the  below when we can change.
  ...
  Gcad::ErrorStatus setHyperlink(const GCHAR* pszName, const GCHAR* pszDescription, const GCHAR* pszSubLocation = NULL);
  ...
  Gcad::ErrorStatus setData(const GCHAR* pszKey, const GcValue* pData, bool bRecursive);
...
{
  GcString sFieldCode;
  return ::gcutGcStringToGChar(sFieldCode, pszFieldCode, this->getFieldCode(sFieldCode, nFlag, pChildFields, mode));
}
...
inline Gcad::ErrorStatus GcDbField::getHyperlink(
  GCHAR** pszName, GCHAR** pszDescription, GCHAR** pszSubLocation, GCHAR** pszDisplayString, Gsoft::Int32* pFlag) const
{
  GcString sName, sDescription, sSubLocation, sDisplayString;
  const Gcad::ErrorStatus es = this->getHyperlink(pszName == nullptr ? nullptr : &sName,
...
GCCORE_PORT bool gcdbFindField(const GCHAR* pszText, int iSearchFrom, int& nStartPos, int& nEndPos);
...
{
  GcString sFieldCode;
  return ::gcutGcStringToGChar(
    sFieldCode, pszFieldCode, ::gcdbMakeFieldCode(pszFieldExpr, sFieldCode, pszEvalId, pszFormat, pHyperlink));
}
