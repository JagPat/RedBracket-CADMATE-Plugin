...
#  define GCRXVALUE_ASSERT(x)                                       \
    if (!(x))                                                       \
      gcutAssertMessage(GCRX_T(#x), GCRX_T(__FILE__), __LINE__, 0); \
    else
#endif
...
public:
  ...
  const IGcRxNonBlittableType* nonBlittable() const
  {
    return m_pNonBlittable;
  }
  const IGcRxEnumeration* enumeration() const
  {
    return m_pEnum;
  }
  const IGcRxReferenceType* reference() const
  {
    return m_pRef;
  }
  const IGcRxObjectValue* rxObjectValue() const
  {
...
protected:
  GCBASE_PORT GcRxValueType(const GCHAR* name,
                            const GCHAR* parent,
                            const IGcRxNonBlittableType& nonBlittable,
                            unsigned int size,
  ...
  GCBASE_PORT GcRxValueType(const GCHAR* name,
                            ...
                            void* userData = NULL);
  GCBASE_PORT GcRxValueType(const GCHAR* name,
                            const IGcRxNonBlittableType& pNonBlittable,
                            unsigned int size,
                            GcRxMemberCollectionConstructorPtr memberConstruct,
                            void* userData = NULL);
  GCBASE_PORT GcRxValueType(const GCHAR* name,
...
template<>
struct GcRxValueType::Desc<void>
{
