...
namespace GcDbSymbolUtilities
{
  ...
  public:
    enum
    {
      kCurrentVersion = GCDBSYMUTIL_SERVICES_CURRENT_VERSION
    };
    ...
    virtual Gcad::ErrorStatus makeDependentName(GCHAR*& pNewName,
                                                const GCHAR* dwgName,
                                                const GCHAR* symbolName) const = 0;
    virtual Gcad::ErrorStatus repairPreExtendedSymbolName(GCHAR*& pNewName,
                                                          const GCHAR* oldName,
                                                          bool allowVerticalBar) const = 0;
    virtual Gcad::ErrorStatus repairSymbolName(GCHAR*& pNewName, const GCHAR* oldName, bool allowVerticalBar) const = 0;
    virtual bool splitDependentName(size_t& numLeftBytes, const GCHAR*& pRight, const GCHAR* symbolName) const = 0;
    virtual Gcad::ErrorStatus validatePreExtendedSymbolName(const GCHAR* name, bool allowVerticalBar) const = 0;
    virtual Gcad::ErrorStatus validateSymbolName(const GCHAR* name, bool allowVerticalBar) const = 0;
    virtual bool compatibilityMode(GcDbDatabase* pDb) const = 0;
    virtual Gcad::ErrorStatus getBlockNameFromInsertPathName(GCHAR*& pBlockName, const GCHAR* pathName) const = 0;
    virtual Gcad::ErrorStatus getInsertPathNameFromBlockName(GCHAR*& pPathName, const GCHAR* blockName) const = 0;
    virtual bool getMaxSymbolNameLength(size_t& maxLength,
                                        size_t& maxSize,
                                        bool isNewName,
                                        bool compatibilityMode) const = 0;
    virtual Gcad::ErrorStatus getPathNameFromSymbolName(GCHAR*& pPathName,
                                                        const GCHAR* symbolName,
                                                        const GCHAR* extensions) const = 0;
    virtual Gcad::ErrorStatus getSymbolNameFromPathName(GCHAR*& pSymbolName,
                                                        const GCHAR* pathName,
                                                        const GCHAR* extensions) const = 0;
    virtual Gcad::ErrorStatus preValidateSymbolName(GCHAR*& pSymbolName, bool preserveCase) const = 0;
    virtual Gcad::ErrorStatus validateCompatibleSymbolName(const GCHAR* name,
                                                           bool isNewName,
...
#define GCDBSYMUTIL_SERVICESNAME_WITH_VERSION(n, v) GCDBSYMUTIL_SERVICESNAME_WITH_VERSION_1(n, v)
#define GCDBSYMUTIL_SERVICES_NAME \
  GCDBSYMUTIL_SERVICESNAME_WITH_VERSION(servicesPtr, GCDBSYMUTIL_SERVICES_CURRENT_VERSION)
...
#define DBSYMUTL_MAKE_GETSYMBOLID_FUNCTION(T_TABLE)                                                    \
  inline Gcad::ErrorStatus get##T_TABLE##Id(GcDbObjectId& objId, const GCHAR* name, GcDbDatabase* pDb) \
  {                                                                                                    \
    if (pDb == NULL)                                                                                   \
...
#define DBSYMUTL_MAKE_HASSYMBOLID_FUNCTION(T_TABLE)                     \
  inline bool has##T_TABLE(GcDbObjectId objId, GcDbDatabase* pDb)       \
  {                                                                     \
    if (pDb == NULL)                                                    \
      return false;                                                     \
...
#define DBSYMUTL_MAKE_HASSYMBOLNAME_FUNCTION(T_TABLE)                   \
  inline bool has##T_TABLE(const GCHAR* name, GcDbDatabase* pDb)        \
  {                                                                     \
    if (pDb == NULL)                                                    \
      return false;                                                     \
    bool hasSymbol = false;                                             \
...
inline const GcDbSymUtilServices* gcdbSymUtil()
{
