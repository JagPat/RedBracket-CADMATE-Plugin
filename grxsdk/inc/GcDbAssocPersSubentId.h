...
public:
  ...
  virtual GcDb::SubentType subentType(const GcDbEntity* pEntity, GcDbDatabase* pDatabase) const = 0;
  virtual int transientSubentCount(const GcDbEntity* pEntity, GcDbDatabase* pDatabase) const;
  virtual bool isNull() const = 0;
  virtual bool isEqualTo(const GcDbEntity* pEntity, GcDbDatabase* pDatabase, const GcDbAssocPersSubentId* pOther) const;
  virtual Gcad::ErrorStatus mirror(const GcDbEntity* pMirroredEntity, GcDbDatabase* pDatabase);

  static Gcad::ErrorStatus createObjectAndDwgInFields(GcDbDatabase* pDatabase,
                                                      GcDbDwgFiler* pFiler,
                                                      GcDbAssocPersSubentId*& pCreatedPersSubentId);
  static Gcad::ErrorStatus createObjectAndDxfInFields(GcDbDxfFiler* pFiler,
                                                      GcDbAssocPersSubentId*& pCreatedPersSubentId);
