...
public:
  ...
  DBCONSTRAINTS_PORT Gcad::ErrorStatus setLevelCount(int nLevels,
                                                     const GcString& expression = GcString(),
                                                     const GcString& evaluatorId = GcString(),
                                                     GcString& errorMessage = dummyString());
  DBCONSTRAINTS_PORT Gcad::ErrorStatus setLevelSpacing(double offset,
                                                       const GcString& expression = GcString(),
                                                       const GcString& evaluatorId = GcString(),
                                                       GcString& errorMessage = dummyString());

  DBCONSTRAINTS_PORT Gcad::ErrorStatus setRowCount(int nRows,
                                                   const GcString& expression = GcString(),
                                                   const GcString& evaluatorId = GcString(),
                                                   GcString& errorMessage = dummyString());
  DBCONSTRAINTS_PORT Gcad::ErrorStatus setRowElevation(double elevation,
                                                       const GcString& expression = GcString(),
                                                       const GcString& evaluatorId = GcString(),
                                                       GcString& errorMessage = dummyString());
  DBCONSTRAINTS_PORT Gcad::ErrorStatus setRowSpacing(double offset,
                                                     const GcString& expression = GcString(),
                                                     const GcString& evaluatorId = GcString(),
                                                     GcString& errorMessage = dummyString());
