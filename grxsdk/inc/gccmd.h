...
#  define gcedRegCmds GcEdCommandStack::cast(gcrxSysRegistry()->at(GCRX_COMMAND_DOCK))
...
public:
  ...
  virtual bool atPut(const GCHAR* cmdGroupName, GcRxDictionary*& cmdsGlobal, GcRxDictionary*& cmdsLocal) = 0;
  ...
  virtual GcEdCommand* lookupCmd(const GCHAR* cmdName, bool globalFlag) = 0;
  virtual GcEdCommand* lookupCmd(const GCHAR* cmdName, bool globalFlag, bool allGroupsFlag) = 0;
  virtual GcEdCommand* lookupCmd(const GCHAR* cmdName, bool globalFlag, bool allGroupsFlag, bool skipUndefed) = 0;
  virtual GcEdCommand* lookupCmd2(
    const GCHAR* cmdName, bool globalFlag, bool allGroupsFlag, int skipFlags, bool match = false) = 0;

  virtual Gcad::ErrorStatus popGroupToTop(const GCHAR* cmdGroupName) = 0;

  virtual Gcad::ErrorStatus removeCmd(const GCHAR* cmdGroupName, const GCHAR* cmdGlobalName) = 0;
