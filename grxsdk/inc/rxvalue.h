...
{
public:
  GcRxValue() throw() : m_type(GcRxValueType::Desc<void>::value())
  {
    memset(&m_value, 0, sizeof(m_value));
  }

  GcRxValue(const GcRxValue& rhs) throw() : m_type(rhs.m_type)
  {
    init(rhs, false);
  }

  GcRxValue(const GcRxValueType& type, const GcRxValue& value) throw() : m_type(type)
  {
    init(value, false);
  }
...
#pragma pop_macro("new")
  ...
  GCBASE_PORT int toString(GCHAR* buffer,
                           size_t sizeInGCHARs,
                           GcRxValueType::StringFormat format = GcRxValueType::kStringFormatGlobal) const throw();
  ...
  template<typename ValueType>
  GcRxValue(const ValueType& value) throw() : m_type(GcRxValueType::Desc<ValueType>::value())
  {
  ...
  {
    return value && GcRxValueType::Desc<ValueType>::value() == value->type() ?
             (ValueType*)(value->valuePtr__<sizeof(ValueType) <= 24>()) :
             0;
  }
  ...
  friend ValueType* rxenum_cast(GcRxValue* value) throw()
  {
    ...
    return value && value->type().isEnum() &&
               GcRxValueType::Desc<ValueType>::value() == value->type().enumeration()->getAt(0).type() ?
             (ValueType*)(value->valuePtr__<sizeof(ValueType) <= 24>()) :
             0;
  }
  template<typename ValueType>
...
#pragma region Implementation
  ...
  GcRxValue(const GcRxValue& rhs, bool realloc) : m_type(rhs.m_type)
  {
    init(rhs, realloc);
  }
...
template<>
inline GcRxValue::GcRxValue(const GCHAR* const& value) throw() : m_type(GcRxValueType::Desc<const GCHAR*>::value())
{
...
#endif
{
  return value && GcRxValueType::Desc<const GCHAR*>::value() == value->type() ? (const GCHAR* const*)&(value->m_value) :
                                                                                0;
}
