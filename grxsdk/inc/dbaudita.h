...
#define GCDB_AUDIT_RETURN(pAuditInfo, baseEs, localEs, boilDownLocalStatus)                       \
  return (baseEs != eOk)     ? baseEs :                                                           \
         (eOk == localEs)    ? eOk :                                                              \
         boilDownLocalStatus ? (pAuditInfo->fixErrors() ? eFixedAllErrors : eLeftErrorsUnfixed) : \
                               localEs;
...
public:
  ...
  void printError(const GCHAR* name, const GCHAR* value, const GCHAR* validation, const GCHAR* defaultValue);
  void printError(const GcDbObject* pObj, const GCHAR* value, const GCHAR* validation, const GCHAR* defaultValue);
