...
class ATL_NO_VTABLE GsdkPromptBase : public CComObjectRootEx<CComSingleThreadModel>,
...
protected:
  ...
  void SetDocument(GcApDocument* pDoc)
  {
    m_pDoc = pDoc;
  }
    ...
    for (int i = 0; i < numFuncs; i++)
    {
      ...
      for (int ii = 0; ii < GetPromptMapSize(); ii++)
      {
        if (memberID == pPromptMap[ii].m_dispId)
        {
          ...
          pPromptMap[ii].m_paramType =
            propertyTypeCallback(ii, pFuncDesc->lprgelemdescParam[nReturnedNames - 1].tdesc.vt);
        }
      }
    }
    return (S_OK);
  ...
  HRESULT PutProperty(int index)
  {
    ...
    HRESULT hr =
      pDisp->Invoke(pPromptMap[index].m_dispId, IID_NULL, LOCALE_USER_DEFAULT, wflags, &dispparams, NULL, NULL, NULL);
  ...
  virtual GcEdJig::DragStatus samplerCallback(int index, bool& compareChange)
  {
    return (kOther);
  }
...
#define PROMPT_ENTRY_INITGET(                                                                  \
  szDesc, dispId, promptType, initget, keywordsId, defaultValueId, promptId, bJigThisProperty) \
